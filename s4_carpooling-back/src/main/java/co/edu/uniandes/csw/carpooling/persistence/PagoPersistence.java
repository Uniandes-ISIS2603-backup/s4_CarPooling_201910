...
public class PagoPersistence {
     ...
     */
    public PagoEntity create (PagoEntity pagoEntity){em.persist(pagoEntity); return pagoEntity; }
    
    /**
     * Busca un registro de la base de datos
     * @param pagoId El id del registro que se está buscando.
     * @return pagoId Si encuentra el registro, devuelve la entidad correspondiente.
     */
    public PagoEntity find (Long pagoId){return em.find(PagoEntity.class, pagoId);}
    
    /**
     * Devuelve todos los registros que se encuentran en la tabla PagoEntity.
     * @return Una lista de entidades.
     */
    public List<PagoEntity> findAll(){TypedQuery<PagoEntity> query = em.createQuery("select u from PagoEntity u", PagoEntity.class); return query.getResultList();}
     ...
     */
    public PagoEntity update(PagoEntity pagoEntity){ return em.merge(pagoEntity);}
    
    /**
     * Borra la entidad con el id que se pasa por parámetro.
     * @param pagoId Id del registro a eliminar.
     */
    public void delete(Long pagoId){PagoEntity entity = em.find(PagoEntity.class, pagoId); em.remove(entity);}
}
